...
void main() {
    ...
    vec3 x_up_normal_mix = mix(te_in[0].v_vertexPosition, te_in[3].v_vertexPosition, gl_TessCoord.x);
    vec3 x_down_normal_mix = mix(te_in[1].v_vertexPosition, te_in[2].v_vertexPosition, gl_TessCoord.x);
    te_out.v_vertexPosition = mix(x_down_normal_mix, x_up_normal_mix, gl_TessCoord.y);

    vec3 x_up_texture_mix = mix(te_in[0].v_vertexColors, te_in[3].v_vertexColors, gl_TessCoord.x);
    vec3 x_down_texture_mix = mix(te_in[1].v_vertexColors, te_in[2].v_vertexColors, gl_TessCoord.x);
    te_out.v_vertexColors = mix(x_down_texture_mix, x_up_texture_mix, gl_TessCoord.y);

    vec3 x_up_position_mix = mix(te_in[0].v_vertexNormals, te_in[3].v_vertexNormals, gl_TessCoord.x);
    vec3 x_down_position_mix = mix(te_in[1].v_vertexNormals, te_in[2].v_vertexNormals, gl_TessCoord.x);
    te_out.v_vertexNormals = mix(x_down_position_mix, x_up_position_mix, gl_TessCoord.y);
